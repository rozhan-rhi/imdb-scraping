...
class Celeb_New:
    def find_new(self,celeb_name):
        ...
        self.news_parts=self.response.find_all("article",class_="ipl-zebra-list__item news-article")
        for self.each_part in self.news_parts:
            if self.celeb_name.lower() in self.each_part.a.text.lower():
                self.date=self.each_part.find("li",class_="ipl-inline-list__item news-article__date").text.replace("\n"," ").strip()
                self.news=self.each_part.find("div",class_="news-article__content").text.strip()
                break  
