...
class Filters_Sub(Filter_Base,Link_Base) :
    ...
    def common_part(self,url) :
        ...
        obj.parse_page(url)    #parse url in parameter
        self.total_info=obj.several_movie(obj.main_page,obj.movie_detail) #use several_movie method to find movies' names and  their contents 
    ...
    def movie_name_filter(self,name):
        ...
        obj.parse_page(self.url) #parse url 
        self.total_info=obj.one_movie(obj.main_page,obj.movie_detail)   #use one_movie method to find movie's name and its content
        return self.total_info
